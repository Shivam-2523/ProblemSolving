...
class Solution {
  public:
    vector<int> bellmanFord(int V, vector<vector<int>>& edges, int src) {
        vector<int> dist(V, 108); // All unreachable by default
        dist[src] = 0;
...
The shortest path between any two nodes can have at most V-1 edges (if no cycles).
More than V-1 edges means a cycle is being used—in case of negative cycles, 
the distance can decrease without bound.
*/
