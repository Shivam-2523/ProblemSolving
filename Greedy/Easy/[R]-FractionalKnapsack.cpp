...
class Solution {
  public:
    ...
    double fractionalKnapsack(vector<int>& val, vector<int>& wt, int capacity) {
        ...
        double sum = 0;
        int weight = 0;
        while(!pq.empty() && weight + wt[pq.top().second]<=capacity)
        {
            sum += val[pq.top().second];
            weight += wt[pq.top().second];
            pq.pop();
        }
        if(!pq.empty() && weight<capacity)
            sum += ((double)val[pq.top().second]/wt[pq.top().second])*(capacity-weight);
        return round(sum * 1000000) / 1000000;
    }
};
